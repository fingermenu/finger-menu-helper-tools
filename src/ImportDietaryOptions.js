        ...
        await BluebirdPromise.each(splittedRows.toArray(), rowChunck =>
          Promise.all(
            rowChunck.map(async rawRow => {
              ...
              const dietaryOptions = await Common.loadAllDietaryOptions(user, { tagId: tag.first().get('id') });
              const info = Map({
                ownedByUser: user,
              ...
              if (dietaryOptions.isEmpty()) {
                ...
                await dietaryOptionService.create(info, acl, null, true);
              } else if (dietaryOptions.count() > 1) {
