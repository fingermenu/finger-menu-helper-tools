          ...
          Promise.all(
            rowChunck.map(async rawRow => {
              ...
              if (servingTimes.isEmpty()) {
                ...
                await servingTimeService.create(info, acl, null, true);
              } else if (servingTimes.count() > 1) {
                console.error(`Multiple serving time found with username ${values.get('username')} and tag name: ${values.get('tagName')}`);
              }
            }),
          ),
        );
