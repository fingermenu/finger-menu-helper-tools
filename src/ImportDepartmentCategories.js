...
const start = async () => {
  try {
    ...
    const departmentCategoryService = new DepartmentCategoryService();
        ...
        await BluebirdPromise.each(splittedRows.toArray(), rowChunck =>
          Promise.all(
            rowChunck.map(async rawRow => {
              ...
              const departmentCategories = await Common.loadAllDepartmentCategories(user, { tagId: tag.first().get('id') });
              const info = Map({
                ownedByUser: user,
              ...
              if (departmentCategories.isEmpty()) {
                ...
                await departmentCategoryService.create(info, acl, null, true);
              } else if (departmentCategories.count() > 1) {
                console.error(`Multiple department categories found with username ${values.get('username')} and tag name: ${values.get('tagName')}`);
              }
            }),
          ),
        );
